...
const Forms = {
  ...
  renderIncidents(insp) {
    ...
    const incidentCards = incidents.map((inc, i) => {
      const photos = this.photos.filter(p => (p.incidentIndex === i));
        ...
        <div class="incident-card" data-incident="${i}">
          ...
          <div class="incident-photos">
            ...
            <label class="photo-add-btn" for="incident-photo-${i}" title="Add photo">
              <span>&#43; Photo</span>
              <input type="file" id="incident-photo-${i}" accept="image/*" capture="environment" class="hidden" data-incident-photo="${i}">
            </label>
          </div>
        </div>`;
    });
  ...
  bindEvents(container) {
    ...
    container.querySelectorAll('[data-incident-photo]').forEach(input => {
      input.addEventListener('change', async (e) => {
        const file = e.target.files[0];
        if (!file) return;
        const idx = parseInt(input.dataset.incidentPhoto);
        ...
        reader.onload = async (ev) => {
          ...
          if (!this.currentInspection.incidents[idx].photoIds) {
            this.currentInspection.incidents[idx].photoIds = [];
          }
          this.currentInspection.incidents[idx].photoIds.push(photo.id);
          await Store.saveInspection(this.currentInspection);
          this.photos.push(photo);
          // Re-render incidents section only
          const incSection = container.querySelector('#section-incidents');
          if (incSection) {
            incSection.innerHTML = container.querySelector('#section-incidents').innerHTML;
          }
          this.saveAndRefresh(container);
        };
